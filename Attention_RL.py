...
while True:  # Run until solved
    ...
    if episode_count % 10 == 0:
        with open('result/attention_training_reward.txt', 'a') as f:
            f.write(f"{episode_count},{time.time()-time_start},{running_reward}")
            f.write('\n')

    if episode_count % 2500 == 0:
        model.save(f"attention_model/{episode_count}_checkpoint")

    if running_reward > 8:  # Condition to consider the task solved
        print("Solved at episode {}!".format(episode_count))
        break

model.save(f"attention_model/final_checkpoint")